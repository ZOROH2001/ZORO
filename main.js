...
const connectionOptions = {
  ...
  browser: ['sokuna&ZORO', 'Safari', '1.0.0'],
  version,
...
let prekey = []
let directorio = readdirSync("./MyZOROSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./MyZOROSession/${files}`)
})
} 
...
const directories = ['./MyZOROSession/', './jadibts/']
...
setInterval(async () => {
  ...
  const bio = `🤖 مده النشاط: ${uptime} ┃ 👑 من صنع سوكونا و فينوم ┃ 🔗 حساب المطورين: https://www.atom.bio/sokuna-ZORO-shadow`;
